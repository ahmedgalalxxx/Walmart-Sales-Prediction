...
from model_evaluation import WalmartModelEvaluator print("=" * 70)
print("WALMART SALES PREDICTION - AUTOMATED RUN")
print("=" * 70) # 1. Data Preprocessing
...
X_train, X_test, y_train, y_test = preprocessor.split_data(test_size=0.2, random_state=42) # Scale (exclude datetime and categorical columns)
exclude_cols = ['Store', 'Holiday_Flag', 'Year']
X_train_scaled, X_test_scaled = preprocessor.scale_features(X_train, X_test, exclude_cols=exclude_cols) print(f"\n Preprocessing Complete!")
print(f" Training set: {X_train_scaled.shape}")
print(f" Test set: {X_test_scaled.shape}") # 2. Model Training
print("\n[2/3] TRAINING MODELS...")
print("-" * 70)
trainer = WalmartModelTrainer(models_dir='models')
trainer.initialize_models()
trained_models = trainer.train_all_models(X_train_scaled, y_train) # 3. Model Evaluation
print("\n[3/3] EVALUATING MODELS...")
print("-" * 70)
evaluator = WalmartModelEvaluator()
evaluator.evaluate_all_models(trained_models, X_train_scaled, y_train, X_test_scaled, y_test) # Display results
print("\n" + "=" * 70)
...
print(test_results.to_string()) # Find best model
...
best_rmse = test_results.loc[f'{best_model_name} (Test)', 'RMSE'] print(f"\n{'=' * 70}")
print(f" BEST MODEL: {best_model_name}")
print(f"{'=' * 70}")
print(f" R² Score: {best_r2:.4f}")
print(f" MAE: ${best_mae:,.2f}")
print(f" RMSE: ${best_rmse:,.2f}")
print(f"{'=' * 70}") # Save models
print("\n[4/4] SAVING MODELS...")
print("-" * 70)
trainer.save_all_models() # Save scaler
import joblib
joblib.dump(preprocessor.scaler, 'models/scaler.joblib')
print(" Scaler saved: models/scaler.joblib") # Save feature names
with open('models/feature_names.txt', 'w') as f: for feature in X_train_scaled.columns: f.write(f"{feature}\n")
print(" Feature names saved: models/feature_names.txt") print("\n" + "=" * 70)
