...
print(f"✓ Data prepared: {X_train_scaled.shape[0]} training, {X_test_scaled.shape[0]} test samples")

# 2. Load the best model (Gradient Boosting)
print("\n[Step 2/5] Loading best model...")
validator = ModelValidator(model_path='models/gradient_boosting_model.joblib')

# 3. Data Quality Validation
print("\n[Step 3/5] Validating data quality...")
data_quality_ok = validator.validate_data_quality(X_test_scaled, y_test)

if not data_quality_ok:
    print("\n⚠ WARNING: Data quality issues detected. Proceeding with caution...")

# 4. Cross-Validation
print("\n[Step 4/5] Performing cross-validation...")
cv_scores = validator.cross_validate(X_train_scaled, y_train, cv=5, scoring='r2')

# 5. Time Series Cross-Validation
print("\n[Step 5/5] Performing time series cross-validation...")
ts_scores = validator.time_series_validation(X_train_scaled, y_train, n_splits=5)

# 6. Prediction Validation
print("\n[Step 6/7] Validating predictions on test set...")
y_pred = validator.validate_predictions(X_test_scaled, y_test, confidence_level=0.95)

# 7. Residual Analysis
print("\n[Step 7/7] Performing residual analysis...")
validator.residual_analysis(X_test_scaled, y_test, y_pred)

# 8. Generate Report
print("\n[Final] Generating validation report...")
validator.generate_validation_report(save_path='results/validation_report.txt')
